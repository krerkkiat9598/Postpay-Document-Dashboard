...
function renderOverviewHighlight(rows){
 ...
 const scopeTitle=scope==="All W&W"?"ภาพรวม W&W":`ตัวกรองปัจจุบัน: ${esc(scope)}`;
 const status=s.cases?`ตรวจ <b>${fmt(s.cases)}</b> รายการ: Complete <b>${pct1(cr)}</b> (${fmt(s.complete)}), Incomplete <b>${pct1(ir)}</b> (${fmt(s.incomplete)}), X Flag <b>${pct1(xr)}</b> (${fmt(s.xflag)})`:`ไม่พบข้อมูลตามตัวกรองปัจจุบัน`;
 ...
 $("overviewHighlight").innerHTML=`<div class="highlight-head"><div class="highlight-icon">★</div><div class="highlight-title">EXECUTIVE HIGHLIGHT • ${scopeTitle}</div></div><div class="highlight-grid"><div class="highlight-item risk"><b>1. QUALITY STATUS — สถานการณ์คุณภาพ</b><br>${status}<br>${quality}<br><b>Executive Takeaway:</b> ${s.incomplete?`ต้องเร่งปิด Incomplete ${fmt(s.incomplete)} รายการ โดยเริ่มจากจุดที่มี Incomplete สูงสุด เพื่อดัน Complete Rate กลับสู่ ≥ ${target}.0%`:`คุณภาพผ่านเป้าหมายและไม่พบ Incomplete Case`}</div><div class="highlight-item goodbox"><b>2. PERFORMANCE — ผลงานและจุดที่ต้อง Focus</b><br>${perf}<br><b>มุมมองผู้บริหาร:</b> ใช้สาขาที่ Complete Rate สูงเป็น Benchmark และ Focus จุดที่มี Incomplete สูงตามตัวกรองปัจจุบัน</div><div class="highlight-item focus"><b>3. ROOT CAUSE & ACTION — สาเหตุและสิ่งที่ควรดำเนินการ</b><br>${rootText}<br>${reasonText}<br>${focusLine}<br><b>Management Action:</b> ${action}</div></div>`;
}
...
function renderEmployee(rows){
 ...
 <div class="highlight-grid">
  ...
  <div class="highlight-item ${us.incomplete?'risk':'goodbox'}"><b>3) คุณภาพข้อมูลผู้รับผิดชอบ:</b> ${us.cases?`มี ${fmt(us.cases)} รายการที่ไม่ระบุพนักงาน • Incomplete ${fmt(us.incomplete)} รายการ • ครอบคลุม ${fmt(ush)} สาขา`:"ไม่พบรายการที่ไม่ระบุพนักงานตามตัวกรองปัจจุบัน"}</div>
 </div>`;
}
...
function renderRoot(rows){
 ...
 <div class="highlight-grid">
  <div class="highlight-item risk"><b>1) สาเหตุหลักของปัญหา:</b> ${topCat?`<b>${esc(topCat[0])}</b> จำนวน <b>${fmt(topCat[1])}</b> รายการ คิดเป็น <b>${pct1(pct(topCat[1],rootDen))}</b> ของ Incomplete ตามตัวกรองปัจจุบัน`:"ไม่มี Incomplete Case"}</div>
